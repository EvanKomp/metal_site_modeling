# metalsitenn/placer_modules/geometry.py
'''
See original code at: https://github.com/baker-laboratory/PLACER/blob/main/modules/geometry.py
'''
import torch
...
def get_frames(a : torch.Tensor,
    ...
    Returns:
        ...
        ex=[...,:,0] - (a->b) x (b->c), perpendicular to the abc plane
        ey=[...,:,1] - ez x ex
...
def triple_prod(a : torch.Tensor,
    ...
    if norm==True:
        ...
        bxc = torch.cross(input = F.normalize(b, dim=-1),
                          other = F.normalize(c, dim=-1),
                          dim = -1)
